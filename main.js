...
var SCREEN_WIDTH  =  window.parent.screen.availWidth;//window.innerWidth;//1600; // 画面横サイズ
var SCREEN_HEIGHT = window.parent.screen.availHeight;//.height;//window.innerHeight;//1200; // 画面縦サイズ
...
var SHARE_MESSAGE = 'Meditation';
var SHARE_HASH_TAGS = 'Meditation';
...
phina.define('Particle', {
  ...
  update: function(app) {
    if (app.pointers.length == 0 ) return;
    flag = (flag + 1) % app.pointers.length;
    var p  = app.pointers[flag];
...
phina.define('MainScene', {
  ...
  init: function(options) {
    ...
    this.life1 = phina.display.Label(
      {text:"", 
      fontSize: 128, 
      fill:"black",//"white"  // Time表示
      }).addChildTo(this);
    this.life1.x = this.gridX.center();//-32;///2;
    this.life1.y = 64;
...
//    var color = ['red', 'blue', 'yellow', 'green', 'white'];
    ...
    this.finger = [];
    for (var i = 0; i < 10; i++) {
        options.fill = 'transparent';//color[i % 5];
  ...
  update: function() {
    ...
    var ps = getfingers(p, 0,0,SCREEN_WIDTH,SCREEN_HEIGHT);
    for (var i = 0; i < 10; i++) {
        if (i < p.length) {
    ...
    elasped = Math.floor((new Date().getTime() - starttime.getTime())/1000);
    this.life1.text = elasped;
